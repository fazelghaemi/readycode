...
function rshf_register_snippet_cpt(){
    ...
    $args = [
        ...
        'public'             => false,
        'show_ui'            => true,
        'show_in_menu'       => false, // we add submenu manually under main menu
...
function rshf_snippet_meta_keys(){
    return [
        'type',      // php, css, js, html
        'location',  // header, body, footer, manual
        'active',    // 1|0
        'priority',  // int
    ];
}
...
function rshf_get_snippet_meta($post_id){
    ...
    $meta = [];
    foreach($keys as $k){
        $meta[$k] = get_post_meta($post_id, 'rshf_' . $k, true);
    }
    // defaults
...
function rshf_save_snippet_meta($post_id){
    if( !isset($_POST['rshf_meta_nonce']) || !wp_verify_nonce($_POST['rshf_meta_nonce'], 'rshf_save_meta') ){
        return;
    }
