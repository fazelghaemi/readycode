...
function rshf_snippet_custom_column($column, $post_id){
    ...
    if($column === 'rshf_type'){
        $t = strtoupper($meta['type']);
        $ico = strtolower($meta['type']);
        echo '<span class="rshf-ico ' . esc_attr($ico) . '" aria-hidden="true"></span> ' . esc_html($t);
    } elseif($column === 'rshf_location'){
        $loc = $meta['location'];
        $ico = in_array($loc, ['header','body','footer'], true) ? $loc : 'info';
        echo '<span class="rshf-ico ' . esc_attr($ico) . '" aria-hidden="true"></span> ' . esc_html($loc);
    }
